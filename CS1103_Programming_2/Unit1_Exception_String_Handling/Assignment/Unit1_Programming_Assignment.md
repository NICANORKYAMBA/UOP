<!--
APA 7 TITLE PAGE (for the Word/PDF export, this content goes on its own page,
centered in the upper half, double-spaced):

                    Programming Assignment Unit 1: Text Analysis Tool
...
Create a text analysis tool that performs various operations on a given text input to help
users gain insights into the text data through character and word analysis.

## Overview

This program is a text analysis tool written in Java. It reads a paragraph of text from
the user and reports the total character count, the total word count, the most common
character, the frequency of a user-chosen character, the frequency of a user-chosen word,
and the number of unique words. Character and word comparisons are case-insensitive, and
surrounding punctuation is ignored during word analysis so that "mat" and "mat." are
treated as the same word. All user input is validated: the paragraph cannot be empty, the
character entry must be exactly one non-blank character, and the word entry must be a
single word. The main workflow is wrapped in a try/catch/finally block so that any
unexpected runtime error is reported cleanly and the `Scanner` resource is always closed.

## Tasks Implemented

1. **User Input** — the program asks the user to enter a paragraph of text and stores it.
2. **Character Count** — displays the total number of characters in the text.
3. **Word Count** — displays the total number of words (words separated by spaces).
4. **Most Common Character** — displays the most frequent character; ties are broken by
   selecting one of the tied characters.
5. **Character Frequency** — asks the user for a character and displays how many times it
   occurs in the text (case-insensitive, so 'a' and 'A' are the same).
6. **Word Frequency** — asks the user for a word and displays how many times it occurs
   (case-insensitive).
7. **Unique Words** — displays the number of unique words in the text (case-insensitive).
...
**Input validation example** (invalid entries are rejected and requested again)
```
Enter a single character to count its frequency:
abc
Invalid input: enter only ONE character, not "abc".
...
**Screenshot 1 — Program running with sample input and full output:**

`[ Screenshot here ]`

**Screenshot 2 — Input validation (invalid entry rejected, then re-prompted):**
