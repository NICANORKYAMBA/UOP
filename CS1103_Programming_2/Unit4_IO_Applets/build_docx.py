#!/usr/bin/env python3
"""
Generate the .docx for the CS 1103 Unit 4 discussion post (I/O Streams and
Applets) from the Markdown source. Forum posts have no title page, so the .md and .docx never drift apart. Double-spaced Times
...
Output: Discussion/Unit4_Discussion_Post.docx
"""
...
SPACING = 1.5  # matches the earlier CS 1103 forum posts
TITLE_PAGE = False  # forum posts: title heading only, no cover page
MONO = "Courier New"
...
JOBS = [
    ("Discussion/Unit4_Discussion_Post.md", "Discussion/Unit4_Discussion_Post.docx"),
]
...
def title_page(doc, title):
    ...
    page_numbers(doc)
    para(doc, "", block=False)
    for line in (l for l in (AUTHOR, AFFIL, COURSE, INSTRUCTOR, DUE) if l):
        para(doc, line, align=WD_ALIGN_PARAGRAPH.CENTER, block=False)
    doc.add_page_break()
    head(doc, title, level=1, center=True)


def finish(doc, body, out):
    in_refs = False
